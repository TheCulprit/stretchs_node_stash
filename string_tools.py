...
@invocation(
    "string_collection_joiner_invocation",
    title="String Collection Joiner",
    tags=["string"],
    category="string",
    version="1.0.1",
)
class StringCollectionJoinerInvocation(BaseInvocation):
    ...
    delimiter: str = InputField(
        title="Delimiter", description="The character to place between each string.", default=", "
    )
    collection: list[str] = InputField(title="Collection", description="The string collection to join.")
    escape_delimiter: bool = InputField(
        title="Escape Delimiter", description="Wehter we should escape the delimiter", default=False
    )
...
@invocation(
    "load_text_file_to_string_invocation",
    title="Load Text File to String",
    tags=["string"],
    category="string",
    version="1.0.0",
    use_cache=False,
...
@invocation(
    ...
    title="Load All Text Files In Folder",
    tags=["string"],
    category="string",
    version="1.0.1",
    use_cache=False,
...
class LoadAllTextFilesInFolderInvocation(BaseInvocation):
    ...
    extension_to_match: str = InputField(
        title="File Extension", description="Only files with the given extension will be loaded. For example: json"
    )
...
@invocation(
    "merge_string_collections_invocation",
    title="Merge String Collections",
...
class MergeStringCollectionsInvocation(BaseInvocation):
    ...
    collection1: Union[str, list[str]] = InputField(
        title="Collection 1", description="A collection of strings or a single string."
    )
    collection2: Union[str, list[str]] = InputField(
        title="Collection 2", description="A collection of strings or a single string."
    )
...
@invocation(
    "string_to_collection_splitter_invocation",
    title="String to Collection Splitter",
    tags=["string"],
    category="string",
    version="1.0.0",
)
class StringToCollectionSplitterInvocation(BaseInvocation):
    ...
    unescape_delimiter: bool = InputField(
        title="Escape Delimiter", description="Whether we should unescape the delimiter", default=False
    )
    ...
    def split_with_delimiter(self, joined_string, delimiter) -> list[str]:
        ...
        while i < len(joined_string):
            ...
            if joined_string[i : i + len(delimiter)] == delimiter:
                ...
                if (
                    i < len(joined_string) - 1
                    and joined_string[i] == "\\"
                    and joined_string[i + 1 : i + 1 + len(delimiter)] == delimiter
                ):
